...
$$\Bigl(\left\lbrace c_{t}^{\ast} \, \ell_{t}^{\ast}\right\rbrace_{t=0}^{\infty},\quad \left\lbrace k_{t}^{*} \, n_{t}^{\ast}\right\rbrace_{t=0}^{\infty},\quad \left\lbrace w_{t}^{\ast} \, r_{t}^{\ast} \, p_{t}^{\ast}\right\rbrace_{t=0}^{\infty}\Bigr)$$
...
3. All markets clear at every date: goods ($c_{t}^{\ast} + i_{t}^{\ast} = y_{t}^{\ast}$), labor ($n_{t}^{s\ast} = n_{t}^{d\ast}$), and money (demand equals supply).
