...
$$\Bigl(\{c_t^*,\, \ell_t^*\}_{t=0}^\infty,\quad \{k_t^*,\, n_t^*\}_{t=0}^\infty,\quad \{w_t^*,\, r_t^*,\, p_t^*\}_{t=0}^\infty\Bigr)$$

of household plans for consumption $c_t^*$ and leisure $\ell_t^*$, firm plans for capital $k_t^*$ and labor demand $n_t^*$, and sequences of factor prices and the price level, such that:
...
3. All markets clear at every date: goods ($c_t^* + i_t^* = y_t^*$), labor ($n_t^{s*} = n_t^{d*}$), and money (demand equals supply).
