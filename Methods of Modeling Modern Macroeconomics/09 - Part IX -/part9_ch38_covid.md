# Chapter 38: Modeling the COVID-19 Pandemic

*Epidemic-Economic Integrated Models*

> *"The pandemic created an externality that standard macroeconomic models cannot capture: your consumption decision imposes infection risk on others. That externality is the source of the market failure."*
...
**Definition 38.1 (SIR Model).** At each date $t$, the population is divided into three compartments:
- $S_t$: **Susceptible** — can contract the disease.
- $I_t$: **Infectious** — currently infected and contagious.
- $R_t$: **Recovered/Removed** — immune or deceased.

With $S_t + I_t + R_t = 1$ (normalized population), the dynamics:

$$\dot{S}_t = -\beta^{epi} S_t I_t, \qquad \dot{I}_t = \beta^{epi} S_t I_t - \gamma I_t, \qquad \dot{R}_t = \gamma I_t.$$

**Definition 38.2 (Basic Reproduction Number).** The **basic reproduction number** $\mathcal{R}_0 = \beta^{epi}/\gamma$ is the expected number of secondary infections generated by one infectious individual in a fully susceptible population. The epidemic grows ($\dot{I} > 0$) iff $\mathcal{R}_0 S_t > 1$.

**Theorem 38.1 (Herd Immunity Threshold).** The epidemic peaks when $S_t = 1/\mathcal{R}_0$. The **herd immunity threshold** (the fraction that must be immune to stop growth) is:

$$HIT = 1 - \frac{1}{\mathcal{R}_0}.$$

*Proof.* The epidemic peaks when $\dot{I} = 0$: $\beta^{epi}S_tI_t - \gamma I_t = 0 \Rightarrow S_t = \gamma/\beta^{epi} = 1/\mathcal{R}_0$. The HIT is $1 - S^{peak} = 1 - 1/\mathcal{R}_0$. $\square$

For COVID-19: $\mathcal{R}_0 \approx 2.5$ (original strain), giving $HIT \approx 60\%$. For the Delta variant: $\mathcal{R}_0 \approx 6$–$7$, $HIT \approx 83$–$86\%$.
...
$$u(C_t, N_t) = \ln C_t - \frac{N_t^{1+\phi}}{1+\phi},$$

subject to the budget constraint and the infection probability that depends on economic activity.

**Infection technology.** The probability that a susceptible household becomes infected depends on its own consumption $C_t$ and labor $N_t$, and on the aggregate infection rate $I_t$:

$$\pi_t^{infection} = \pi^C C_t I_t + \pi^N N_t I_t + \pi^{other} I_t,$$

where $\pi^C, \pi^N, \pi^{other}$ are transmission rates from consumption contacts, work contacts, and other contacts. This specification makes infection endogenous to economic choices — a key departure from exogenous epidemiological models.

**Modified SIR dynamics:**

$$\dot{S}_t = -(\pi^C C_t + \pi^N N_t + \pi^{other})I_t S_t,$$
$$\dot{I}_t = (\pi^C C_t + \pi^N N_t + \pi^{other})I_t S_t - \gamma I_t.$$

The effective transmission rate $\beta^{epi}_t = \pi^C C_t + \pi^N N_t + \pi^{other}$ is now **endogenous** — it falls when households voluntarily reduce activity.
...
$$V^S(a_t) = \max_{C_t, N_t}\left\{u(C_t, N_t) + e^{-\rho\Delta t}\left[(1-\pi_t)V^S(a_{t+1}) + \pi_t V^I(a_{t+1})\right]\right\},$$

where $\pi_t$ is the infection probability and $V^I$ is the value of being infected (lower, due to illness costs).

**The modified Euler equation for susceptible households:**

$$u_C(C_t) = e^{-\rho}\!\left[(1-\pi_t)V^S_C(a_{t+1}) + \pi_t V^I_C(a_{t+1}) + \text{infection externality correction}\right].$$
...
**The contagion externality.** The social planner solves the same problem but includes the effect of each household's activity on the aggregate infection rate $I_t$:

$$\text{Externality} = \frac{\partial\pi^{aggregate}}{\partial C_t}\cdot(\text{value loss from infecting others}).$$

The externality is **negative** (more activity imposes more infection risk on the susceptible population) and grows with $I_t$ (more infectious people → higher external cost of activity).
...
$$\max_{\{C_t, N_t\}}\int_0^\infty e^{-\rho t}\left[S_t u(C_t, N_t) + I_t u^I(C_t^I, N_t^I) + R_t u(C_t^R, N_t^R)\right]dt$$

subject to SIR dynamics with endogenous transmission.

**The Hamiltonian:**

$$\mathcal{H} = S\cdot u + I\cdot u^I + R\cdot u^R + \lambda^S\dot{S} + \lambda^I\dot{I} + \lambda^R\dot{R},$$

where $\lambda^S$, $\lambda^I$, $\lambda^R$ are the costate variables (shadow values of the susceptible, infected, and recovered populations).

**Theorem 38.2 (Optimal Lockdown Condition).** The social planner's optimality condition for consumption:

$$u_C(C_t) = \underbrace{\lambda^S_t\cdot\pi^C I_t (S_t - 1)}_{\text{internalized infection cost}} + \underbrace{e^{-\rho}\mathbb{E}[V^S_C]}_{\text{standard Euler}},$$

where the additional term captures the cost of infecting others through consumption contacts. The social planner's optimal consumption is lower than the competitive equilibrium whenever $I_t > 0$ — the optimal lockdown is an internalization of the contagion externality.

*Derivation.* The costate equation for $\lambda^S$: $\dot\lambda^S = \rho\lambda^S - \partial\mathcal{H}/\partial S$. The FOC for $C_t$: $\partial\mathcal{H}/\partial C = 0$, giving $S_tu_C(C_t) + \lambda^S(-\pi^C I_t S_t) + \lambda^I(\pi^C I_t S_t) = 0$, so the shadow benefit of consumption ($Su_C$) equals the net shadow cost of infection spread $\lambda^S - \lambda^I$ times the contact rate. $\square$

---

## 38.6 Numerical Solution of the ERT Model

The ERT model is solved as a **two-point boundary value problem**: initial conditions fix $(S_0, I_0, R_0)$ (epidemiological state at pandemic onset), while a terminal condition (herd immunity or vaccine arrival) fixes the endpoint.

**Algorithm 38.1 (ERT Solution).**

1. Discretize time into periods of 1 week (or 1 day for precision).
2. At the optimal lockdown, the household's FOC pins $(C_t, N_t)$ as functions of the state $(S_t, I_t)$ and costate $(\lambda^S_t, \lambda^I_t)$.
3. The costate equations $\dot\lambda^S, \dot\lambda^I$ provide the costate dynamics.
4. The system $(S, I, R, \lambda^S, \lambda^I)$ is solved by the shooting method: guess $\lambda^S_0$, integrate forward, adjust until the terminal condition is satisfied.
...
The ERT model shows the optimal lockdown begins when the infection rate exceeds a threshold $I^*$ determined by the costate equation:

$$I^* \approx \frac{u_C(C^*)}{(\lambda^S - \lambda^I)\cdot\pi^C\cdot S^*},$$
...
When a vaccine arrives at date $T_V$ with coverage $\nu$ (fraction vaccinated), the effective $S$ drops to $(1-\nu)S_{T_V}$. If $\nu > HIT$, herd immunity is achieved and the epidemic ends. Optimal vaccine rollout prioritizes the most active (high-contact) individuals — workers in customer-facing industries — rather than the most at-risk (elderly), balancing epidemiological effectiveness with distributional equity.

### 38.7.3 Fiscal Stimulus

The CARES Act ($\approx$\$2 trillion) provided direct payments to households and expanded unemployment insurance. In the ERT model, direct payments increase consumption — which increases transmission if $I_t > 0$. The optimal fiscal response targets consumption that has low transmission risk (home food delivery rather than restaurant meals) while supporting incomes.
...
Implement the SIR model in APL as a scan: `{sir_step ⍵}\ T ⍴ ⊂(S0, I0, R0)`. (a) Simulate 100 weeks with $\mathcal{R}_0 = 2.5$. (b) Compute the HIT numerically: find $t^*$ where $\dot{I} = 0$ and verify $S_{t^*} = 1/\mathcal{R}_0$. (c) Simulate with $30\%$ voluntary reduction in contacts (multiply $\beta$ by $0.7$) and compare the peak infection rate.

### Exercise 38.2 (Python — Vaccine Arrival Timing)

Simulate the ERT model with a vaccine arriving at $T_V \in \{26, 52, 78, 104\}$ weeks (6, 12, 18, 24 months) with $\nu = 0.7$ coverage. (a) For each $T_V$, compute: total infections before vaccine, GDP loss, and welfare. (b) Plot the trade-off between earlier vaccine arrival and total pandemic cost. (c) Estimate the value of a 6-month acceleration in vaccine development (the "vaccine acceleration premium") in welfare units.
...
- The **SIR model** partitions the population into $(S, I, R)$ with dynamics $\dot{S} = -\beta SI$, $\dot{I} = \beta SI - \gamma I$; the basic reproduction number $\mathcal{R}_0 = \beta/\gamma$ determines whether the epidemic grows; herd immunity threshold $HIT = 1-1/\mathcal{R}_0$ (Theorem 38.1).
- The **ERT model** makes the transmission rate endogenous: $\beta_t = \pi^C C_t + \pi^N N_t + \pi^0$; households voluntarily reduce activity to avoid infection but do not internalize the infection externality.
...
- The **optimal lockdown condition** (Theorem 38.2): social planner's FOC for consumption includes an externality correction proportional to $(\lambda^S - \lambda^I)\pi^C I_t$ — the shadow cost of spreading infection multiplied by the contact rate.
